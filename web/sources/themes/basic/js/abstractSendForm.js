...
AbstractSendForm.prototype.htmlSend = function(event, container, cleanFields=false, closestError=false, timeoutRemove=false) {
    try {
        this.baseSend(event, success);
...
AbstractSendForm.prototype.htmlArraySend = function(event, container1, container2, item1, item2, disable=false, cleanFields=false, closestError=false, timeoutRemove=false) {
    try {
        this.baseSend(event, success);
        ...
        function success(data, status, jqXHR) {
            ...
            if (typeof data == 'object' && data.length != 0) {
                if (data.hasOwnProperty(item1) && data.hasOwnProperty(item2)) {
                    ...
                    if (timeoutRemove == true) {
                        this.timeoutRemove(container1, 5000);
                    }
                } else {
                    if (closestError == true) {
                        this.addClosestErrors(data);
