...
class AddressByAddressMapperTests extends \PHPUnit_Framework_TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        $command = \Yii::$app->db->createCommand('INSERT INTO {{address}} SET [[id]]=:id, [[address]]=:address, [[city]]=:city, [[country]]=:country');
        $command->bindValues([':id'=>self::$_id + 1, ':address'=>self::$_address, ':city'=>self::$_city, ':country'=>self::$_country]);
        $command->execute();
    ...
    public function testGetGroup()
    {
        ...
        $this->assertFalse(empty($objectAddress->id));
        $this->assertFalse(empty($objectAddress->address));
        $this->assertFalse(empty($objectAddress->city));
        $this->assertFalse(empty($objectAddress->country));
        $this->assertFalse(empty($objectAddress->postcode));
    }
    ...
    public function testGetGroupWithoutPostcode()
    {
        ...
        $this->assertEquals(self::$_id + 1, $objectAddress->id);
    }
