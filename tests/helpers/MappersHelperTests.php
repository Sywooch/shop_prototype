...
class MappersHelperTests extends \PHPUnit_Framework_TestCase
{
    ...
    private static $_total_products = 263;
    ...
    /**
     * Тестирует метод MappersHelper::getCurrencyList
     */
    public function testGetCurrencyList()
    {
        ...
        $result = MappersHelper::getCurrencyList();
    ...
    public function testGetColorsList()
    {
        ...
        $command->bindValues([':id'=>self::$_id, ':date'=>self::$_date, ':code'=>self::$_code, ':name'=>self::$_name, ':description'=>self::$_description, ':short_description'=>self::$_description, ':price'=>self::$_price, ':images'=>self::$_images, ':id_categories'=>self::$_id, ':id_subcategory'=>self::$_id, ':active'=>self::$_active, ':total_products'=>self::$_total_products]);
        $command->execute();
    ...
    {
        $command = \Yii::$app->db->createCommand('INSERT INTO {{products}} SET [[id]]=:id, [[date]]=:date, [[code]]=:code, [[name]]=:name, [[description]]=:description, [[price]]=:price, [[images]]=:images, [[id_categories]]=:id_categories, [[id_subcategory]]=:id_subcategory, [[active]]=:active, [[total_products]]=:total_products');
        $command->bindValues([':id'=>self::$_id + 23, ':date'=>self::$_date, ':code'=>self::$_code . 'n', ':name'=>self::$_name, ':description'=>self::$_description, ':price'=>self::$_price, ':images'=>self::$_images, ':id_categories'=>self::$_id, ':id_subcategory'=>self::$_id, ':active'=>self::$_active, ':total_products'=>self::$_total_products]);
        $command->execute();
    ...
    public function testSetProductsInsert()
    {
        ...
        $this->assertEquals(self::$_total_products, $result[0]['total_products']);
    }
    ...
    /**
     * Тестирует метод MappersHelper::getCommentsList
     */
    public function testGetCommentsList()
    {
        if (empty(\Yii::$app->db->createCommand('SELECT * FROM {{comments}}')->queryAll())) {
            
            if (empty($id = \Yii::$app->db->createCommand('SELECT [[id]] FROM {{emails}} LIMIT 1')->queryScalar())) {
                $id = self::$_id;
                $command = \Yii::$app->db->createCommand('INSERT INTO {{emails}} SET [[id]]=:id,[[email]]=:email');
                $command->bindValues([':id'=>$id, ':email'=>self::$_email]);
                $command->execute();
            }
            ...
            $command->bindValues([':id'=>self::$_id, ':text'=>self::$_text, ':name'=>self::$_name, ':id_emails'=>$id, ':id_products'=>self::$_id, ':active'=>self::$_active]);
            $command->execute();
        }
        
        $this->assertFalse(empty(\Yii::$app->db->createCommand('SELECT * FROM {{comments}}')->queryAll()));
        
        $result = MappersHelper::getCommentsList();
    ...
    /**
     * Тестирует метод MappersHelper::setCurrencyInsert
     */
    public function testSetCurrencyInsert()
    {
        if (!empty(\Yii::$app->db->createCommand('SELECT * FROM {{currency}}')->queryAll())) {
            \Yii::$app->db->createCommand('DELETE FROM {{currency}}')->execute();
        }
        $this->assertTrue(empty(\Yii::$app->db->createCommand('SELECT * FROM {{currency}}')->queryAll()));
        
        $currencyModel = new CurrencyModel();
        $currencyModel->currency = self::$_currency;
        $currencyModel->exchange_rate = self::$_exchange_rate;
        $currencyModel->main = self::$_main;
        
        $result = MappersHelper::setCurrencyInsert([$currencyModel]);
    ...
    /**
     * Тестирует метод MappersHelper::getCurrencyByCurrency
     */
    public function testGetCurrencyByCurrency()
    {
        $currencysModel = new CurrencyModel();
        $currencysModel->currency = self::$_currency;
        
        $result = MappersHelper::getCurrencyByCurrency($currencysModel);
    ...
    /**
     * Тестирует метод MappersHelper::setCurrencyUpdateMainNull
     */
    public function testSetCurrencyUpdateMainNull()
    {
        $this->assertFalse(empty($currency = \Yii::$app->db->createCommand('SELECT * FROM {{currency}}')->queryOne()));
        $this->assertTrue((bool) $currency['main']);
        
        $result = MappersHelper::setCurrencyUpdateMainNull();
